...
public class CircleProgressView extends View implements View.OnClickListener {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        circleSize = a.getDimensionPixelSize(R.styleable.CircleProgressView_inner_circle_size, 0);
        textSize = a.getDimensionPixelSize(R.styleable.CircleProgressView_inner_text_size, 0);
        circleColor = a.getDimensionPixelSize(R.styleable.CircleProgressView_inner_circle_color, 0);
        textColor = a.getDimensionPixelSize(R.styleable.CircleProgressView_inner_text_color, 0);
        strokeWidth = a.getDimensionPixelSize(R.styleable.CircleProgressView_stroke_width, 0);
        strokeColor = a.getDimensionPixelSize(R.styleable.CircleProgressView_stroke_color, 0);
        strokeMargin = a.getDimensionPixelSize(R.styleable.CircleProgressView_stroke_margin, 0);
    ...
    private void initDefaultValueWhenEmpty(Context context) {
        if (circleSize == 0) circleSize = dipToPx(context, 30f);
        if (textSize == 0) textSize = 16;
        if (circleColor == 0) circleColor = 0xafffffff;
        if (textColor == 0) textColor = Color.WHITE;
        if (strokeWidth == 0) strokeWidth = dipToPx(context, 2f);
        if (strokeColor == 0) strokeColor = 0xafffffff;
        if (strokeMargin == 0) strokeMargin = dipToPx(context, 5f);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!isFailed) {
            ...
            canvas.drawCircle(circleRect.centerX(), circleRect.centerY(), radius + strokeMargin, strokePaint);
    ...
    public synchronized void setCurrentPresent(int currentPresent) {
        ...
        if (DEBUG) {
            Log.d(TAG, "" + this.currentPresent + "    id>>>>>>>>   " + currentThreadId);
        }
